...
var cityArr = [];
cityArr[0] = ['东城区', '西城区','崇文区','宣武区','朝阳区','丰台区','石景山区', '海淀区','门头沟区', '房山区','通州区','顺义区','昌平区','大兴区','怀柔区','平谷区','密云县','延庆县'];
...
cityArr[10] = ['合肥市', '芜湖市', '蚌埠市', '淮南市', '马鞍山市', '淮北市', '铜陵市', '安庆市', '黄山市', '滁州市','阜阳市','宿州市', '巢湖市', '六安市', '亳州市', '池州市', '宣城市'];
...
function set_city()
{
    ...
    if(pv=="none") return;
    if(typeof(cityArr[pv])=='undefined') return;

    for(var i=0; i<cityArr[pv].length; i++)
    {
       city.options[i+1] = new Option();
       city.options[i+1].text = cityArr[pv][i];
       city.options[i+1].value = cityArr[pv][i];
    }
    //var x=document.getElementsByTagName("option");
    //x.style.backgroundColor = "rgba(256,256,256,0.6)";
